...
const selectedBranchNameEl = document.getElementById('selectedBranchName');
...
function applyFilters() {
    ...
    window.location.href = query ? `${createUrl}?${query}` : createUrl;
}

function getCurrentBranchName() {
    if (selectedBranchNameEl && selectedBranchNameEl.textContent.trim()) {
        return selectedBranchNameEl.textContent.trim();
    }

    const selectedBranchOption = branchIdEl && branchIdEl.tagName === 'SELECT'
        ? branchIdEl.options[branchIdEl.selectedIndex]
        : null;

    return selectedBranchOption && selectedBranchOption.value
...
function fillConfirmationModal() {
    ...
    confirmBranchNameEl.textContent = getCurrentBranchName();
...
function buildConfirmMessage() {
    ...
    return [
        'Xác nhận đặt sân?',
        `Chi nhánh: ${getCurrentBranchName()}`,
